...
public class FocusableObject : InteractionBase, IFocusable
{
    ...
    {
        // Think a better way to clear destroyed objects
        if (_subInteractableObjects.Any(x => x == null))
            _subInteractableObjects = _subInteractableObjects.Where(x => x != null).ToArray();
