...
public abstract class PuzzleParent : InteractionBase, IFocusable
{
    ...
    [SerializeField] private bool cooldownAllPieces;
    [SerializeField] private bool manualCompletionCheck;
    ...
    public bool CooldownAllPieces => cooldownAllPieces;
    ...
    {
        if(!manualCompletionCheck) return;
        CheckCompletion();
    }
    ...
    public void StartPiecesCooldown()
    {
        for (int i = 0; i < _puzzlePieces.Length; i++)
        {
            _puzzlePieces[i].StartCooldown();
        }
    }
